...
import { ensureAuth } from "../../../server/http/middlewares/auth";
...
export const meetingsRouter = () => {
  ...
  const findAllMeetingsController = new FindAllMeetingsController(
    meetingsRepository
  );
  const createMeetingController = new CreateMeetingController(
    meetingsRepository
  );
  const findOneMeetingController = new FindOneMeetingController(
    meetingsRepository
  );
  const deleteMeetingController = new DeleteMeetingController(
    meetingsRepository
  );
  ...
  meetingsRouter.get(
    "/",
    ensureAuth,
    async (req, res) => await findAllMeetingsController.handle(req, res)
  );
  meetingsRouter.get(
    "/:id",
    async (req, res) => await findOneMeetingController.handle(req, res)
  );
